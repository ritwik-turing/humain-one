...
2. Open the current prototype and click through all nine screens: [Eval_Journey_V9.html](file:///Users/ritwikmac/Documents/GitHub/humain-one/Eval_Journey_V9.html) , live at https://ritwik-turing.github.io/humain-one/Eval_Journey_V9.html
...
4. Read the coverage ledger, which is the Prism demo checked item by item against V9: [PRISM_DEMO_COVERAGE.md](file:///Users/ritwikmac/Documents/GitHub/humain-one/PRISM_DEMO_COVERAGE.md)
...
6. Before you change anything, run the self test so you know the baseline is green: `bash "/Users/ritwikmac/Documents/GitHub/humain-one/tools/selftest.sh" "/Users/ritwikmac/Documents/GitHub/humain-one/Eval_Journey_V9.html"`
...
- Uday Kumar Pabbathi, Prism POC. Has been sent V5; V9 supersedes it. Reviewing the mockup.
...
Every metric declares the one part it cannot work without: the trace, the output, a prose output, an expected answer, reference steps. Whether it runs is answered per case, so coverage is a count shown on screen: accuracy runs on 349 of 380, helpfulness on 161, and the product says so instead of quietly averaging over a third of the run. This is the one idea everything else hangs off.

The nine screens, in order:

1. Set up an evaluation, in three steps mirroring Prism's Create Evaluation. Dataset from CSV, documents or JSON, a Drive link, a warehouse, a bucket, or sampled production traffic; then name the fields with a role dropdown per key, Input, Expected output, Metadata, Ignore, because Prism has no schema file. Unmap the expected output and accuracy becomes unavailable with the reason and drops out of the count. Then the metric library, tabbed by Prism's four kinds, Prompt, Deterministic, REST API, Agentic, with search and a builder for each: a prompt with variables whose coverage is derived from the variables used, values to run it once, saved as v1.0.0; a Python function with Prism's evaluate(input, expected_output, agent_response) contract, data input from any dataset row, sandbox run; an endpoint with the posted payload and the score field; an agentic call. Then the evaluation: name, description, the agent as a typed REST API object with its endpoint, number of runs 1 to 5, parallel datapoints, and a functional metric calibration tab showing each judge metric's agreement with the developer's own verdicts or "not yet checked against a human".
...
- **Not built, on purpose** (from the ledger): Import, Collections and list filters, the Metrics Overview radar, dataset download, attaching documents to an agent, metrics attached at the agent level. Comparing two different evaluations rather than two jobs is the one partial row worth building next.
...
- Uday did not open Prism's Metric calibration tab in the recording. V9 shows the tab and the agreed human-verdict workflow without inventing hidden fields.
- Internal transport batch size and retry policy remain Chandra's engineering implementation detail. V9 settles the product behavior: each datapoint is evaluated independently, independent datapoints can be delivered in parallel, and each mapped input is checked against the agent limit before the run.
...
|---|---|---|
| [Eval_Journey_V9.html](file:///Users/ritwikmac/Documents/GitHub/humain-one/Eval_Journey_V9.html) | CURRENT prototype. Cross-evaluation selectors drive the comparison, unmatched cases and metric-set differences remain visible, performance fields match the walkthrough, and Edit plus setup-tab navigation work. | [https://ritwik-turing.github.io/humain-one/Eval_Journey_V9.html](https://ritwik-turing.github.io/humain-one/Eval_Journey_V9.html) |
...
| [PRISM_DEMO_COVERAGE.md](file:///Users/ritwikmac/Documents/GitHub/humain-one/PRISM_DEMO_COVERAGE.md) | The ledger: every item from Uday's Prism demo against V9, built, covered at the journey boundary, or excluded by decision. No walkthrough item is unaccounted for. | [https://github.com/ritwik-turing/humain-one/blob/main/PRISM_DEMO_COVERAGE.md](https://github.com/ritwik-turing/humain-one/blob/main/PRISM_DEMO_COVERAGE.md) |
...
### The project folder, the canonical copy of the explainer and all earlier research
