...
var HB_REFS=[['none','Input field from the dataset','Is the reply polite and does it answer every part?','Runs on every case'],['expected','Input and expected answer fields from the dataset','Does the reply agree with the expected answer?','Runs on cases that have an expected answer'],['record','Input, expected answer and a supporting field from the dataset','Is every claim supported by the policy text?','Runs on cases where that field is filled']];
...
function hbRender(){
  ...
  else{evidence='<div class="hb-cards hb-cards-3">'+HB_REFS.map(function(x){return '<button type="button" class="hb-card hb-ev'+(ref===x[0]?' on':'')+'" data-hb-ref="'+x[0]+'" aria-pressed="'+(ref===x[0])+'"><small class="hb-ev-needs">This metric needs</small><b>'+x[1]+'</b><span class="hb-ev-eg">e.g. “'+x[2]+'”</span><span class="hb-ev-runs">'+x[3]+'</span></button>'}).join('')+'</div>'
  ...
  host.innerHTML='<div class="hb"><div class="hb-main">'
    ...
    +'<section class="hb-sec"><span class="hb-n">2</span><div><h4>What does it need from the dataset?</h4>'+evidence+'</div></section>'
...
function hrToolbarHTML(){
  ...
  return '<div class="hr-toolbar"><span class="hr-toolbar-sel">'+(n?n+' selected':'Select a run')+'</span>'
    ...
    +'<button type="button" class="btn ghost sm" id="hrEvidence"'+(one?'':' disabled')+'>Review evidence</button>'
...
(function(){
  ...
  window.addEventListener('click',function(e){
    ...
    else if(t.id==='hrEvidence'){showPacket(RUN_SEL[0]);}
...
AGENT_CATALOG.push({slug:'agentic-invoice-processing',name:'Invoice Processing Agent',versions:[['v4.1.3','previous'],['v4.1.4','draft · unpublished']]});
...
function invTrace(){return [['ok','Orchestrator accepted the document','0.2s','','CHAIN','main'],['ok','Read the invoice image','0.6s','','TOOL','extract_invoice'],['ok','Extracted the fields','0.8s','','LLM','extract_invoice'],['ok','Matched the vendor in the master list','0.5s &middot; 1 match','','RETRIEVER','vendor_reconciliation'],['ok','Returned the answer','0.3s','','CHAIN','main']];}
var INV_EXP_TRACE=['Read the invoice image','Extracted the fields','Matched the vendor in the master list','Returned the answer'];
function invCase(i,kind){
  var r=INV_VENDORS[i%INV_VENDORS.length],file='invoice_'+String(410+i)+'.jpg',exp=invJSON(r[0],r[1],r[2],r[3]),act=exp,fin=1,loop=1,out=1,t='2.6s',tag='',tr=invTrace();
  if(kind==='wrongtotal'){act=invJSON(r[0],r[1],r[2],'1,205.00');out=2;tag='misread';tr[2]=['bad','Extracted the fields','0.9s','Read the subtotal line instead of the grand total.','LLM','extract_invoice'];}
  if(kind==='novat'){act=invJSON(r[0],r[1],r[2],r[3].replace(/^([0-9,]+)\.(\d+)$/,function(m,a,b){return a+'.'+b}));act=act.replace('SAR','SAR (excl. VAT)');out=2;tag='incomplete';tr.splice(3,0,['bad','Skipped the VAT line','0.3s','The 15% VAT line was in the footer and not added.','LLM','extract_invoice']);}
  if(kind==='unreadable'){act='Nothing came back.';fin=0;loop=0;out=0;t='31.0s';tag='noreader';tr=[['ok','Orchestrator accepted the document','0.2s','','CHAIN','main'],['ok','Read the invoice image','0.6s','','TOOL','extract_invoice'],['bad','Tried text recognition again','repeated 6 times · 29.8s','attempt 1 ... blurred scan\nattempt 2 ... blurred scan','TOOL','extract_invoice'],['bad','Timed out','no fields returned','','CHAIN','main']];}
  if(kind==='duplicate'){act=invJSON(r[0],INV_VENDORS[(i+1)%INV_VENDORS.length][1],r[2],r[3]);out=2;tag='misread';tr[2]=['bad','Extracted the fields','0.8s','Took the PO number as the invoice number.','LLM','extract_invoice'];}
  if(kind==='currency'){act=invJSON(r[0],r[1],r[2],r[3]).replace('SAR','USD');out=2;tag='misread';tr[2]=['bad','Extracted the fields','0.8s','Currency symbol misread as USD; the invoice is in SAR.','LLM','extract_invoice'];}
  if(kind==='slow'){t='9.8s';tr[2]=['ok','Extracted the fields','7.9s','Handwritten total; two passes needed.','LLM','extract_invoice'];}
  if(kind==='wrongvendor'){act=invJSON('Ministry of Finance · Accounts Payable',r[1],r[2],r[3]);out=2;tag='misread';tr[2]=['bad','Extracted the fields','0.8s','Took the bill-to name (the buyer) as the vendor.','LLM','extract_invoice'];tr[3]=['bad','Matched the vendor in the master list','0.7s · 0 matches','No vendor record for "Ministry of Finance · Accounts Payable".','RETRIEVER','vendor_reconciliation'];}
  return {j:'Invoice · '+r[0],req:'Extract vendor, invoice number, date and total from the invoice image',inp:[{t:'image',v:file,m:'1240 x 1754 · '+(280+i*7)+' KB'}],outp:fin?[{t:'json',v:act,m:'4 fields'}]:[],lab:{t:'json',v:exp,m:'4 fields'},fin:fin,loop:loop,out:out,t:t,tr:tr,expTr:INV_EXP_TRACE.slice(),autoTag:tag,noteUnavailableReason:'',q:file,exp:exp,act:act};
}
function invBackground(i){var r=INV_VENDORS[i%INV_VENDORS.length],file='invoice_'+String(500+i)+'.jpg',exp=invJSON(r[0],r[1].replace(/\d+$/,function(n){return String(+n+i)}),r[2],r[3]);return {j:'Invoice · '+r[0],inT:'image',outT:'json',lang:i%5===0?'AR':'EN',fin:1,loop:1,out:1,t:2.4+(i%7)/5,expTr:true,m:'',req:'Extract vendor, invoice number, date and total from the invoice image',q:file,inp:[{t:'image',v:file,m:'1240 x 1754 · '+(240+i*9)+' KB'}],outp:[{t:'json',v:exp,m:'4 fields'}],lab:{t:'json',v:exp,m:'4 fields'},tr:invTrace()};}
...
function seedDemo(){
  ...
  try{
    ...
    var invKinds=['wrongtotal','novat','unreadable','duplicate','currency','slow','wrongvendor'];
    var invA=invKinds.map(function(k,i){return invCase(i,k)}),invB=[];for(var i=0;i<12;i++)invB.push(invBackground(i));
    var invA2=[invCase(7,'slow'),invCase(8,'clean')],invB2=[];for(var j=0;j<6;j++)invB2.push(invBackground(12+j));
...
(function(){
  ...
  function applyHrScope(){try{var q=new URLSearchParams(location.search);if(q.get('scope')!=='agent'||APP_MODE!=='hub')return;var ag=hrAgentBySlug(q.get('agentslug')||AGENT_SLUG);var ver=q.get('agent')||AGENT_VERSION;ver=/^v/.test(ver)?ver:'v'+ver;
    HR_SCOPE={slug:ag.slug,name:ag.name,ver:ver};AGENT_SLUG=ag.slug;AGENT_NAME=ag.name;AGENT_VERSION=ver;HUB_RUN_AGENT=ag.slug;HUB_RUN_VER=ver;if(q.get('evalset'))HUB_RUN_SET=q.get('evalset');else{var kw={'agentic-invoice-processing':/invoice/i,'invoice-processing':/invoice/i,'document-analysis':/document/i,'al-noor-services':/permits|baseline/i,'permit-renewal':/safety|permit/i}[ag.slug];var hit=kw?BASELINES.filter(function(b){return kw.test(b.name||b.reason||'')}):[];if(hit.length)HUB_RUN_SET=hit[hit.length-1].id;}document.body.classList.add('hr-scoped');}catch(e){}}
...
function hrReviewPayload(r){var rc=r.receipt||{};var ref=/^([^@]+)@(.+)$/.exec(String(rc.agent||''));var sc=HR_SCOPE||{slug:ref?ref[1]:AGENT_SLUG,name:hrAgentBySlug(ref?ref[1]:AGENT_SLUG).name,ver:ref?'v'+ref[2]:AGENT_VERSION};
  ...
  return {slug:sc.slug,name:sc.name,ver:sc.ver,evaluation:r.number,runId:rc.run_id||'',experimentset:hrSetName(rc.baseline&&rc.baseline.id),cases:(rc.coverage&&rc.coverage.attempted)||(r.cases||[]).length,metrics:ms,avg:hrRunAvg(r),flagged:flagged};}
...
function hrSubmitBarHTML(){return '';}
function hrSubmitBarHTML_old(){var runs=hrScopedRuns();if(!runs.length)return '';
...
(function(){
  ...
  otelTrace=function(c,index){var o=_ot.apply(this,arguments);
    ...
    o.spans.forEach(function(s){var m=/repeated (\d+) times/.exec(String(s.summary||''));if(!m)return;
      ...
      s.status='ERROR';s.attrs['error.type']=s.attrs['error.type']||'AgentRetryExhausted';s.attrs['humain.retry.max']=n;if(!(s.events&&s.events.length))s.events=[{name:'exception',attributes:{'exception.type':s.attrs['error.type'],'exception.message':'Gave up after '+n+' attempts: '+(lines[lines.length-1]||s.label||s.name),'exception.escaped':'true'}}];});
    return o;};
...
/* V99: manifest-driven node spans for agentic-invoice-processing (agent.yaml graph: main -> extract_invoice -> vendor_reconciliation) */
var NODE_INFO={main:{agent:'Invoice Orchestrator',path:'/api/research',role:'orchestrator',input:'query',output:'answer'},extract_invoice:{agent:'Invoice Extractor',path:'/api/extract',role:'sub_agent',input:'document',output:'fields'},vendor_reconciliation:{agent:'Vendor Reconciler',path:'/api/reconcile',role:'sub_agent',input:'query',output:'answer'}};
var INV_MANIFEST={name:'agentic-invoice-processing',version:'4.1.4',entry:'main',runtime:'python',commit:'0a36a1b4a7a2b8c6e24fce82715723452335678a',repo:'https://goat-advertisers-plot-turning.trycloudflare.com'};
(function(){
  var _ot2=otelTrace;
  otelTrace=function(c,index){var o=_ot2.apply(this,arguments);var tr=c.tr||[];
    if(!tr.some(function(st){return st[5]}))return o;
    o.root.attrs['humain.manifest.entry']=INV_MANIFEST.entry;o.root.attrs['vcs.ref.head.revision']=INV_MANIFEST.commit;o.root.attrs['vcs.repository.url.full']=INV_MANIFEST.repo;o.root.attrs['humain.manifest.nodes']=3;
    var groups=[];o.spans.forEach(function(s,i){var key=(tr[i]&&tr[i][5])||'main';var g=groups[groups.length-1];if(!g||g.key!==key){g={key:key,leaves:[]};groups.push(g);}g.leaves.push(s);});
    o.spans=groups.map(function(g,gi){var info=NODE_INFO[g.key]||{agent:g.key,path:'/'+g.key,role:'sub_agent',input:'query',output:'answer'};var first=g.leaves[0],last=g.leaves[g.leaves.length-1];var err=g.leaves.some(function(l){return l.status==='ERROR'});
      var node={name:'invoke_agent '+info.agent,label:'node '+g.key+' · POST '+info.path,id:stableHex('node:'+o.root.id+':'+g.key+':'+gi,16),parent:o.root.id,kind:'SERVER',status:err?'ERROR':'UNSET',offset:first.offset,duration:Math.max(1,(last.offset+last.duration)-first.offset),summary:'Manifest node '+g.key+' handled by '+info.agent+' ('+info.role+') on POST '+info.path,
        attrs:{'gen_ai.operation.name':'invoke_agent','gen_ai.agent.name':info.agent,'gen_ai.agent.id':g.key,'http.request.method':'POST','http.route':info.path,'server.address':INV_MANIFEST.name+'.sandbox.humain.internal','http.response.status_code':err?500:200,'humain.node.name':g.key,'humain.node.type':'action','humain.agent.role':info.role,'humain.io.input_field':info.input,'humain.io.output_field':info.output,'humain.eval.case.id':o.root.attrs['humain.eval.case.id']},events:[],children:g.leaves};
      g.leaves.forEach(function(l){l.parent=node.id;l.attrs['humain.node.name']=g.key;});
      return node;});
    return o;};
})();

/* V101: evidence packet. Review evidence opens the packet that goes to the tenant verifier; Submit for review lives only there. */
var PACKET_RUN=null,PKT_ATTEST={},PKT_NOTE={},PKT_SUBMITTED={};
function pktRun(n){return BASE_RUNS.filter(function(r){return r.number===n})[0];}
function pktCases(r){return r.number===RUNS_DONE?CASES:(r.cases||[]);}
function pktAllCases(r){try{if(r.number===RUNS_DONE&&typeof allCases==='function')return allCases();}catch(e){}return (r.cases||[]).concat(r.background||[]);}
function pktReviewed(c){return c.noteState==='accepted'||c.noteState==='edited'||c.noteState==='dismissed';}
function pktAgent(r){var ref=/^([^@]+)@(.+)$/.exec(String((r.receipt||{}).agent||''));var slug=ref?ref[1]:AGENT_SLUG;return {slug:slug,ver:ref?'v'+ref[2]:AGENT_VERSION,name:hrAgentBySlug(slug).name};}
function pktId(r){return 'pkt-'+stableHex('packet:'+r.receipt.run_id,10);}
function pktTraceId(r,i){return stableHex('trace:'+r.receipt.run_id+':'+i,32);}
function pktShort(v,n){v=String(v==null?'':v).replace(/\s+/g,' ').trim();return v.length>n?v.slice(0,n-1)+'…':v;}
function pktMetricsOf(r){return (r.metrics||[]).filter(function(m){return m._available!==false&&(typeof m._score==='number'||!!metricResponseContract(m))});}
function pktContract(r,m){var contract=metricResponseContract(m);if(!contract)return null;var counts={},cases=pktAllCases(r),n=0;cases.forEach(function(c,i){if(!c.fin&&!c.out)return;var l;try{l=resultLabel(m,i);}catch(e){l=null;}if(l==null)return;counts[l]=(counts[l]||0)+1;n++;});if(contract.mode!=='list'){var max=contract.maximum||5,min=contract.minimum||0,sum=0,k=0;Object.keys(counts).forEach(function(l){var v=parseFloat(l);if(!isNaN(v)){sum+=v*counts[l];k+=counts[l];}});var avg=k?sum/k:0;return {counts:counts,n:k,ok:k,pct:k?Math.round((avg-min)/((max-min)||1)*100):0,good:'average '+avg.toFixed(1)+' of '+max,range:true};}var good=(contract.responses||[]).filter(function(x){return !x.highlight}).map(function(x){return x.label});var ok=good.reduce(function(a,l){return a+(counts[l]||0)},0);return {counts:counts,n:n,ok:ok,pct:n?Math.round(ok/n*100):0,good:good.join(' / ')};}
function pktScore(r,m){if(typeof m._score==='number')return m._score;var c=pktContract(r,m);return c?c.pct:0;}
function pktPrev(r){var a=pktAgent(r);return BASE_RUNS.filter(function(x){return x.number<r.number&&x.receipt&&x.receipt.baseline&&x.receipt.baseline.id===r.receipt.baseline.id&&String(x.receipt.agent||'').split('@')[0]===a.slug}).pop()||null;}
function pktBetter(r){var a=pktAgent(r),me=hrRunAvg(r);return BASE_RUNS.filter(function(x){return x.number!==r.number&&String(x.receipt.agent||'')===String(r.receipt.agent||'')&&x.receipt.baseline.id===r.receipt.baseline.id&&hrRunAvg(x)>me}).sort(function(x,y){return hrRunAvg(y)-hrRunAvg(x)})[0]||null;}
function pktFlagged(r){return pktCases(r).map(function(c,i){return {c:c,i:i}}).filter(function(x){return x.c.autoTag||x.c.tag||!x.c.fin||x.c.out===2||x.c.out===0});}
function pktVisual(r,m){var isLatest=r.number===RUNS_DONE;
  if(isLatest){try{var live=jobMetrics().filter(function(x){return x.k===m.k})[0]||m;if(metricResponseContract(live)){var v=contractSummaryHTML(live);if(v)return v;}var v2=scoreChartHTML(live,m._score+'%');if(v2)return v2;}catch(e){}}
  var cc=pktContract(r,m);if(cc){var tot=cc.n||1;return '<span class="cr-head"><b>'+cc.pct+'%</b><small>'+esc(cc.good)+(cc.range?' · '+cc.n+' cases':' · '+cc.ok+' of '+cc.n+' cases')+'</small></span><span class="cr-seg">'+Object.keys(cc.counts).map(function(l){return '<i class="'+(cc.good.indexOf(l)>-1?'ok':'flag')+'" style="width:'+(cc.counts[l]/tot*100)+'%" title="'+esc(l)+' '+cc.counts[l]+'"></i>'}).join('')+'</span><span class="cr-legend">'+Object.keys(cc.counts).map(function(l){return '<span>'+esc(l)+' '+cc.counts[l]+'</span>'}).join('')+'</span>';}
  var cases=pktAllCases(r),vals=[];cases.forEach(function(c,i){if(!c.fin&&!c.out)return;vals.push(cellScore(i,m.k,c.fin&&c.loop&&c.out===1));});
  var edges=[0,20,40,60,80,101],labels=['0–19','20–39','40–59','60–79','80–100'],counts=[0,0,0,0,0];vals.forEach(function(v){for(var b=0;b<5;b++){if(v>=edges[b]&&v<edges[b+1]){counts[b]++;break;}}});var max=Math.max.apply(null,counts)||1;
  return '<span class="cr-head"><b>'+pktScore(r,m)+'%</b><small>average of '+vals.length+' case scores</small></span><span class="cr-bars cr-bars-5">'+counts.map(function(n,b){return '<i class="'+(b<2?'low':b<3?'mid':'high')+'"><small>'+n+'</small><em style="height:'+Math.max(2,Math.round(n/max*40))+'px"></em><b>'+labels[b]+'</b></i>'}).join('')+'</span>';}
function pktJSON(r){var rc=r.receipt,a=pktAgent(r),fl=pktFlagged(r),cases=pktCases(r);
  return {packet:{id:pktId(r),schema:'humain.evidence-packet/1',generated:new Date().toISOString()},agent:{name:a.name,id:a.slug,version:a.ver,manifest:a.slug==='agentic-invoice-processing'?INV_MANIFEST:null},
    evaluation:{number:r.number,run_id:rc.run_id,created:rc.created,started_at:r.at?new Date(r.at).toISOString():null,environment:rc.environment,settings:rc.settings,coverage:rc.coverage,tokens:rc.tokens},
    experiment_set:{id:rc.baseline.id,name:rc.baseline.name||rc.baseline.reason,version:rc.baseline.version,dataset:rc.dataset,sources:(rc.baseline.sources||[]).map(function(x){return {name:x.name,provider:x.provider,cases:x.count,dataset:x.datasetName,dataset_version:x.datasetVersion,mapping:x.mapping}})},
    metrics:pktMetricsOf(r).map(function(m){var cc=pktContract(r,m);return {key:m.k,name:m.n,kind:m.by,version:m.ver,scope:m.scope,calibration:m._calibration,score:pktScore(r,m),labels:cc?cc.counts:undefined,coverage:m._coverage}}),
    cases:{total:pktAllCases(r).length,flagged:fl.length,items:fl.map(function(x){var c=x.c;return {index:x.i+1,case_id:c.caseId||('case-'+String(x.i+1).padStart(4,'0')),title:c.j,tag:c.autoTag||c.tag||'',reasons:(function(){try{return attentionReasons(c,x.i).filter(function(q){return q.kind!=='control'}).map(function(q){return q.label});}catch(e){return [];}})(),finished:!!c.fin,expected:pktShort(c.exp||(c.lab&&c.lab.v)||'',400),actual:pktShort(c.act||(c.outp&&c.outp[0]&&c.outp[0].v)||'',400),decision:c.noteState||'not reviewed',note:c.note||'',trace_id:pktTraceId(r,x.i)}})},
    human_review:{reviewed:cases.filter(pktReviewed).length,confirmed:cases.filter(function(c){return c.noteState==='accepted'||c.noteState==='edited'}).length,dismissed:cases.filter(function(c){return c.noteState==='dismissed'}).length}};}
function pktHash(obj){try{var enc=new TextEncoder().encode(JSON.stringify(obj));return crypto.subtle.digest('SHA-256',enc).then(function(buf){return Array.prototype.map.call(new Uint8Array(buf),function(b){return ('0'+b.toString(16)).slice(-2)}).join('');});}catch(e){return Promise.resolve('');}}
function pktKV(obj){return '<div class="otel-grid pkt-grid">'+Object.keys(obj).map(function(k){return '<span>'+esc(k)+'</span><span>'+obj[k]+'</span>'}).join('')+'</div>';}
function renderPacket(n){var r=pktRun(n);var box=document.getElementById('hubPacket');if(!r||!box)return;
  var rc=r.receipt,a=pktAgent(r),cases=pktCases(r),fl=pktFlagged(r),mets=pktMetricsOf(r),prev=pktPrev(r),better=pktBetter(r),id=pktId(r),sub=PKT_SUBMITTED[n],att=PKT_ATTEST[n]||{},allAtt=att.a&&att.b&&att.c;
  var allc=pktAllCases(r),langs={};allc.forEach(function(c){var l=c.lang||'EN';langs[l]=(langs[l]||0)+1;});var skipped=(rc.coverage.attempted||0)-(rc.coverage.completed||0);
  var reviewed=cases.filter(pktReviewed).length,confirmed=cases.filter(function(c){return c.noteState==='accepted'||c.noteState==='edited'}).length,dismissed=cases.filter(function(c){return c.noteState==='dismissed'}).length;
  var judges=mets.filter(function(m){return m.by==='prompt'||/prompt|judge/i.test(m.by||'')}),uncal=mets.filter(function(m){return /not calibrated|signal/i.test(m._calibration||'')});
  var toc=[['summary','Summary'],['agent','Agent under test'],['setup','How it was tested'],['metrics','Metrics'],['results','Results'],['cases','Case evidence'],['review','Human review'],['gaps','Known gaps'],['attest','Attestation']];
  var h='<div class="pkt">'
   +'<div class="pkt-top"><button type="button" class="btn ghost sm" id="hrPacketBack">← Evaluations</button><span class="pkt-status '+(sub?'sent':'draft')+'">'+(sub?'Submitted · queued for review':'Draft packet · not yet submitted')+'</span><span class="sp"></span><button type="button" class="btn ghost sm" id="hrPacketExport">Download JSON</button><button type="button" class="btn ghost sm" id="hrPacketPrint">Print</button></div>'
   +'<div class="pkt-cover"><p class="eyebrow">Evidence packet · '+esc(id)+'</p><h2 class="h1">'+esc(a.name)+' '+esc(a.ver)+' · Evaluation '+r.number+'</h2><p class="sub">What the tenant verifier receives: the frozen evaluation, its dataset and metrics, every flagged case with its trace, your review decisions, and the gaps we know about. Nothing here is decided for you; the verifier reads the same numbers you see.</p>'
   +'<div class="pkt-facts"><div><b>Experiment set</b><span>'+esc(hrSetName(rc.baseline.id))+'</span></div><div><b>Run id</b><span class="mono">'+esc(rc.run_id)+'</span></div><div><b>Evaluated</b><span>'+esc(rc.created)+'</span></div><div><b>Cases</b><span>'+rc.coverage.attempted+' attempted · '+rc.coverage.completed+' completed</span></div><div><b>Verifier</b><span>Tenant verifier · Tenant admin team</span></div><div><b>Packet hash</b><span class="mono" id="pktHash">computing…</span></div></div>'
   +(better?'<div class="pkt-hint"><b>Evaluation '+better.number+' scored higher on this set ('+hrRunAvg(better)+' against '+hrRunAvg(r)+').</b> You can send this one, or <button type="button" class="lnk" data-hr-packet="'+better.number+'">open the packet for Evaluation '+better.number+'</button>.</div>':'')
   +'</div>'
   +'<nav class="pkt-toc">'+toc.map(function(t){return '<a href="#pkt-'+t[0]+'">'+t[1]+'</a>'}).join('')+'</nav>';
  h+='<section class="pkt-sec" id="pkt-summary"><h3>1 · Summary</h3><div class="pkt-tiles">'+mets.map(function(m){var d=prev?(function(){var pm=pktMetricsOf(prev).filter(function(x){return x.k===m.k})[0];return pm?pktScore(r,m)-pktScore(prev,pm):null})():null;var cc=pktContract(r,m);return '<div class="pkt-tile"><b>'+pktScore(r,m)+'%</b><span>'+esc(m.n)+'</span><small>'+(cc?(cc.range?cc.good+' · '+cc.n+' cases':cc.good+' · '+cc.ok+' of '+cc.n):(m._coverage?m._coverage.ran+' of '+m._coverage.total+' cases':''))+(d!=null?' · '+(d>=0?'+':'')+d+' vs Evaluation '+prev.number:'')+'</small></div>'}).join('')+'<div class="pkt-tile"><b>'+fl.length+'</b><span>flagged cases</span><small>'+confirmed+' confirmed problems · '+dismissed+' marked acceptable</small></div></div>'
   +'<p class="pkt-p">Decision requested: review '+esc(a.name)+' '+esc(a.ver)+' for certification on the evidence below. Average across metrics '+hrRunAvg(r)+'. There is no platform pass bar; the verifier applies tenant policy.</p></section>';
  var mf=a.slug==='agentic-invoice-processing'?INV_MANIFEST:null;
  h+='<section class="pkt-sec" id="pkt-agent"><h3>2 · Agent under test</h3>'+pktKV(mf?{'Agent':esc(a.name)+' · <span class="mono">'+esc(mf.name)+'</span>','Version':esc(a.ver)+' · frozen at submission','Manifest entry':'<span class="mono">'+mf.entry+'</span> · nodes main → extract_invoice → vendor_reconciliation','Sub-agents':Object.keys(NODE_INFO).map(function(k){var x=NODE_INFO[k];return esc(x.agent)+' · POST '+x.path+' · '+x.input+' → '+x.output}).join('<br>'),'Runtime':mf.runtime+' · cpu 1 · memory 4Gi','Source':'<span class="mono">'+mf.commit.slice(0,12)+'</span> · '+esc(mf.repo),'Secrets':'GEMINI_API_KEY, OPENAI_API_KEY declared by name, values from the tenant vault','Environment':esc(rc.environment.id)+' · '+esc(rc.environment.policy)}:{'Agent':esc(a.name)+' · <span class="mono">'+esc(a.slug)+'</span>','Version':esc(a.ver)+' · frozen at submission','Environment':esc(rc.environment.id)+' · '+esc(rc.environment.policy)})+'</section>';
  h+='<section class="pkt-sec" id="pkt-setup"><h3>3 · How it was tested</h3>'+pktKV({'Experiment set':esc(hrSetName(rc.baseline.id))+' · <span class="mono">'+esc(rc.baseline.id)+'</span>','Dataset':esc(rc.dataset),'Sources':(rc.baseline.sources||[]).map(function(x){return esc(x.provider)+' · <span class="mono">'+esc(x.name)+'</span> · '+x.count+' cases'}).join('<br>'),'Field mapping':Object.keys(rc.field_mapping||{}).filter(function(k){return rc.field_mapping[k]!=='ignore'}).map(function(k){return '<span class="mono">'+esc(k)+'</span> → '+esc(rc.field_mapping[k])}).join(' · '),'Run settings':(rc.settings.repetitions||1)+' run per case · '+(rc.settings.parallel?'parallel':'sequential'),'Coverage':rc.coverage.attempted+' attempted · '+rc.coverage.completed+' completed · '+skipped+' skipped','Languages':Object.keys(langs).map(function(k){return k+' '+langs[k]}).join(' · '),'Tokens':(rc.tokens?rc.tokens.input+' in · '+rc.tokens.output+' out':'n/a'),'Run id':'<span class="mono">'+esc(rc.run_id)+'</span> · '+esc(rc.created)})+'</section>';
  h+='<section class="pkt-sec" id="pkt-metrics"><h3>4 · Metrics</h3><div class="pkt-table-wrap"><table class="pkt-table"><thead><tr><th>Metric</th><th>Kind</th><th>What it checks</th><th>Calibration</th></tr></thead><tbody>'+mets.map(function(m){var cfg=(m.config||{});var what=cfg.prompt||cfg.rule||m.d||'';var contract=metricResponseContract(m);return '<tr><td><b>'+esc(m.n)+'</b><small>'+esc(m.ver||'v1.0.0')+' · '+esc(m.scope||'')+'</small></td><td>'+esc(m.by||'')+(cfg.model?'<small>'+esc(cfg.model)+'</small>':'')+'</td><td>'+esc(pktShort(what,220))+(contract&&contract.mode==='list'?'<small>Answers: '+contract.responses.map(function(x){return esc(x.label)}).join(' / ')+'</small>':'')+'</td><td>'+esc(m._calibration||'')+'</td></tr>'}).join('')+'</tbody></table></div></section>';
  h+='<section class="pkt-sec" id="pkt-results"><h3>5 · Results</h3><div class="pkt-results">'+mets.map(function(m){return '<div class="pkt-metric"><b class="rname">'+esc(m.n)+'</b><span class="rmeta">'+esc(m.by||'')+' · '+esc(m._calibration||'')+'</span><span class="rvalue authored-results cr-auto">'+pktVisual(r,m)+'</span><span class="rcoverage">'+(m._coverage?m._coverage.ran+' of '+m._coverage.total+' scored'+(m._coverage.skipped?' · '+m._coverage.skipped+' skipped: '+esc(m._coverage.why||''):''):'')+'</span></div>'}).join('')+'</div>'+(prev?'<p class="pkt-p">Compared with Evaluation '+prev.number+' on the same experiment set: '+mets.map(function(m){var pm=pktMetricsOf(prev).filter(function(x){return x.k===m.k})[0];var dd=pm?pktScore(r,m)-pktScore(prev,pm):null;return pm?esc(m.n)+' '+(dd>=0?'+':'')+dd:''}).filter(Boolean).join(', ')+'.</p>':'<p class="pkt-p">No earlier evaluation of this agent on this experiment set, so there is no comparison yet.</p>')+'</section>';
  h+='<section class="pkt-sec" id="pkt-cases"><h3>6 · Case evidence</h3><p class="pkt-p">'+fl.length+' of '+allc.length+' cases flagged. Every case carries its OpenTelemetry trace id; the verifier opens the same case view you use.</p><div class="pkt-table-wrap"><table class="pkt-table pkt-cases"><thead><tr><th>#</th><th>Case</th><th>Expected</th><th>Actual</th><th>Flag</th><th>Your decision</th><th>Trace</th></tr></thead><tbody>'+(fl.length?fl.map(function(x){var c=x.c;var dec=c.noteState==='accepted'||c.noteState==='edited'?'<span class="badge b-gov">Problem confirmed</span>':c.noteState==='dismissed'?'<span class="badge b-lock">Acceptable</span>':'<span class="badge b-pre">Not reviewed</span>';return '<tr><td class="ds-n">'+(x.i+1)+'</td><td><b>'+esc(c.j||'')+'</b><small class="mono">'+esc(c.caseId||'')+'</small></td><td class="mono small">'+esc(pktShort(c.exp||(c.lab&&c.lab.v)||'',120))+'</td><td class="mono small">'+esc(pktShort(c.act||(c.outp&&c.outp[0]&&c.outp[0].v)||(c.fin?'':'No output'),120))+'</td><td>'+(function(){var rs=[];try{rs=attentionReasons(c,x.i).filter(function(q){return q.kind!=='control'});}catch(e){}return rs.length?rs.map(function(q){return '<span class="tag">'+esc(q.short||q.label)+'</span>'}).join(' '):'<span class="tag">'+esc(c.autoTag||c.tag||'flagged')+'</span>';})()+'</td><td>'+dec+(c.note?'<small>'+esc(pktShort(c.note,140))+'</small>':'')+'</td><td class="mono small">'+pktTraceId(r,x.i).slice(0,16)+'…'+(r.number===RUNS_DONE?'<br><button type="button" class="lnk" data-hr-case="'+x.i+'">Open case</button>':'')+'</td></tr>'}).join(''):'<tr><td colspan="7" class="ds-empty-cell"><b>No flagged cases</b><span>Every case finished and matched its expected output.</span></td></tr>')+'</tbody></table></div></section>';
  h+='<section class="pkt-sec" id="pkt-review"><h3>7 · Human review</h3>'+pktKV({'Flagged cases reviewed':reviewed+' of '+fl.length,'Problems confirmed':String(confirmed),'Marked acceptable':String(dismissed),'Still unreviewed':String(Math.max(0,fl.length-reviewed)),'Reviewer':'Developer of record · this workspace'})+(fl.length-reviewed>0?'<p class="pkt-p warn">'+(fl.length-reviewed)+' flagged case'+(fl.length-reviewed===1?'':'s')+' still unreviewed. The verifier will see them as unreviewed; you can still submit.</p>':'<p class="pkt-p">Every flagged case has a recorded decision.</p>')+'</section>';
  var gaps=[];if(uncal.length)gaps.push(uncal.map(function(m){return esc(m.n)}).join(', ')+': AI judge '+(uncal.length===1?'metric is':'metrics are')+' not calibrated against human labels. Treat as signal, not a pass bar.');if((rc.settings.repetitions||1)===1)gaps.push('One run per case, so no variance estimate. Re-run with more cycles before reading small differences.');if(skipped)gaps.push(skipped+' case'+(skipped===1?'':'s')+' returned nothing and '+(skipped===1?'was':'were')+' scored as missing, not as wrong.');if(Object.keys(langs).length<2)gaps.push('Single-language dataset ('+Object.keys(langs).join('')+'). Arabic coverage is not evidenced here.');else gaps.push('Language split '+Object.keys(langs).map(function(k){return k+' '+langs[k]}).join(' / ')+'. Per-language results are not broken out in this packet.');gaps.push('Traces and model calls are simulated in this prototype; the trace ids are stable but not from a live collector.');gaps.push('Dataset of '+allc.length+' cases from '+(rc.baseline.sources||[]).length+' source'+((rc.baseline.sources||[]).length===1?'':'s')+'; production traffic may differ.');
  h+='<section class="pkt-sec" id="pkt-gaps"><h3>8 · Known gaps and limitations</h3><ul class="pkt-list">'+gaps.map(function(g){return '<li>'+g+'</li>'}).join('')+'</ul></section>';
  h+='<section class="pkt-sec pkt-attest" id="pkt-attest"><h3>9 · Attestation and submission</h3>'+(sub?'<p class="pkt-p">Submitted to the tenant verifier. Track the judgement in Agent Lifecycle.</p>':'<p class="pkt-p">Signed by the developer of record. The packet hash above is what the verifier checks against.</p>'
   +'<label class="pkt-check"><input type="checkbox" data-hr-att="a"'+(att.a?' checked':'')+'> '+esc(a.ver)+' is the exact package that will be certified; nothing changes between this evaluation and publication.</label>'
   +'<label class="pkt-check"><input type="checkbox" data-hr-att="b"'+(att.b?' checked':'')+'> The dataset was not used to train or tune this agent.</label>'
   +'<label class="pkt-check"><input type="checkbox" data-hr-att="c"'+(att.c?' checked':'')+'> I have read the flagged cases and the known gaps listed above.</label>'
   +'<label class="pkt-note"><span>Note to the verifier (optional)</span><textarea id="hrPacketNote" rows="3" placeholder="What changed in this version, what you want checked, anything the numbers do not say.">'+esc(PKT_NOTE[n]||'')+'</textarea></label>'
   +'<div class="pkt-submit"><button type="button" class="btn" id="hrPacketSubmit"'+(allAtt?'':' disabled')+'>Submit for review</button><span class="muted">'+(allAtt?'Sends this packet to the tenant verifier. One open submission per agent.':'Tick the three statements to enable submission.')+'</span></div>')+'</section></div>';
  box.innerHTML=h;
  pktHash(pktJSON(r)).then(function(hx){var el=document.getElementById('pktHash');if(el&&hx)el.textContent='sha256:'+hx.slice(0,24)+'…';if(el)el.title='sha256:'+hx;PKT_LAST_HASH=hx;});
}
var PKT_LAST_HASH='';
function showPacket(n){var r=pktRun(n);if(!r){toast('Select an evaluation first.');return;}
  var box=document.getElementById('hubPacket');if(!box){var host=document.getElementById('hubRuns');box=document.createElement('section');box.className='screen';box.id='hubPacket';host.parentNode.insertBefore(box,host.nextSibling);}
  PACKET_RUN=n;renderPacket(n);document.querySelectorAll('.screen').forEach(function(x){x.classList.toggle('on',x.id==='hubPacket')});window.scrollTo({top:0,behavior:'instant'});}
function pktSubmit(n){var r=pktRun(n);if(!r)return;var p=hrReviewPayload(r);p.packet={id:pktId(r),hash:PKT_LAST_HASH?'sha256:'+PKT_LAST_HASH:'',note:PKT_NOTE[n]||'',flagged:pktFlagged(r).length,reviewed:pktCases(r).filter(pktReviewed).length};
  PKT_SUBMITTED[n]=true;if(window.parent!==window){shellPost('humain-eval-submit-review',p);toast('Packet '+p.packet.id+' submitted for review.');}else{toast('Packet '+p.packet.id+' would go to the tenant verifier. Open this inside HUMAIN ONE to follow it in Agent Lifecycle.');}renderPacket(n);}
window.addEventListener('click',function(e){var t=e.target.closest&&e.target.closest('#hrPacketBack,#hrPacketExport,#hrPacketPrint,#hrPacketSubmit,[data-hr-packet],[data-hr-case]');if(!t)return;e.preventDefault();e.stopImmediatePropagation();
  if(t.id==='hrPacketBack'){showHubPage('runs');return;}
  if(t.id==='hrPacketPrint'){window.print();return;}
  if(t.id==='hrPacketExport'){var r=pktRun(PACKET_RUN);if(!r)return;var j=pktJSON(r);j.packet.hash=PKT_LAST_HASH?'sha256:'+PKT_LAST_HASH:'';var a=document.createElement('a'),url=URL.createObjectURL(new Blob([JSON.stringify(j,null,2)],{type:'application/json'}));a.href=url;a.download=j.packet.id+'.json';a.click();setTimeout(function(){URL.revokeObjectURL(url)},1000);return;}
  if(t.id==='hrPacketSubmit'){pktSubmit(PACKET_RUN);return;}
  if(t.dataset.hrPacket){showPacket(+t.dataset.hrPacket);return;}
  if(t.dataset.hrCase!=null){toConsole('results');goTrace(+t.dataset.hrCase);return;}},true);
document.addEventListener('change',function(e){var el=e.target;if(!el)return;if(el.dataset&&el.dataset.hrAtt){PKT_ATTEST[PACKET_RUN]=PKT_ATTEST[PACKET_RUN]||{};PKT_ATTEST[PACKET_RUN][el.dataset.hrAtt]=el.checked;var ta=document.getElementById('hrPacketNote');if(ta)PKT_NOTE[PACKET_RUN]=ta.value;renderPacket(PACKET_RUN);}},true);
document.addEventListener('input',function(e){var el=e.target;if(el&&el.id==='hrPacketNote')PKT_NOTE[PACKET_RUN]=el.value;},true);

/* V102: simulated metric results follow the case content (judge labels and the seeded invoice code metrics), one-time demo reseed */
var SEED_REV=4;
function crCaseAt(i){try{return allCases()[i]||null;}catch(e){return null;}}
function crJSON(v){if(!v)return null;if(typeof v==='object')return v;try{return JSON.parse(String(v));}catch(e){return null;}}
function crNorm(s){return String(s==null?'':s).toLowerCase().replace(/\b(llc|ltd|limited|co|company|inc|est|establishment|trading)\b\.?/g,'').replace(/[^a-z0-9؀-ۿ]+/g,' ').trim();}
function crField(m){var t=(((m&&m.n)||'')+' '+((m&&m.config&&(m.config.rule||m.config.prompt))||'')).toLowerCase();if(/vendor|supplier/.test(t))return 'vendor';if(/invoice number|invoice_number/.test(t))return 'invoice_number';if(/total|amount/.test(t))return 'total';if(/\bdate\b/.test(t))return 'date';return null;}
function crCaseIO(c){var exp=c&&(c.exp||(c.lab&&c.lab.v))||'',act=c&&(c.act||(c.outp&&c.outp[0]&&c.outp[0].v))||'';return {exp:exp,act:act,ej:crJSON(exp),aj:crJSON(act),noOutput:!c||(!c.fin&&!c.out)||!act||/^nothing came back/i.test(String(act))};}
(function(){
  var _crf=contractResultFor;
  contractResultFor=function(contract,m,i){
    var c=crCaseAt(i);if(!c||!contract||contract.mode!=='list')return _crf.apply(this,arguments);
    var rs=contract.responses||[],good=rs.filter(function(r){return !r.highlight}),bad=rs.filter(function(r){return r.highlight});if(!good.length||!bad.length)return _crf.apply(this,arguments);
    var io=crCaseIO(c),pick=function(re,list){return (list.filter(function(r){return re.test(r.label+' '+(r.meaning||''))})[0]||list[list.length-1]).label};
    if(io.noOutput)return pick(/unclear|missing|unknown|cannot|not enough|no output/i,bad);
    var f=crField(m);
    if(f&&io.ej&&io.aj){if(!(f in io.aj)||!(f in io.ej))return pick(/unclear|missing|unknown|cannot|not enough/i,bad);return crNorm(io.ej[f])===crNorm(io.aj[f])?good[0].label:pick(/different|mismatch|wrong|not the same|another/i,bad);}
    if(io.ej&&io.aj)return JSON.stringify(io.ej)===JSON.stringify(io.aj)?good[0].label:_crf.apply(this,arguments);
    if(String(io.exp).trim()&&crNorm(io.exp)===crNorm(io.act))return good[0].label;
    return _crf.apply(this,arguments);
  };
  function fieldsScore(c){var io=crCaseIO(c);if(io.noOutput)return 0;if(!io.ej||!io.aj)return crNorm(io.exp)===crNorm(io.act)?100:0;var keys=Object.keys(io.ej);if(!keys.length)return 0;var ok=keys.filter(function(k){return crNorm(io.ej[k])===crNorm(io.aj[k])}).length;return Math.round(ok/keys.length*100);}
  function num(v){var m=String(v==null?'':v).replace(/,/g,'').match(/-?\d+(\.\d+)?/);return m?parseFloat(m[0]):NaN;}
  function cur(s){var m=String(s||'').match(/\b(SAR|USD|EUR|AED)\b/i);return m?m[1].toUpperCase():'';}
  function toleranceScore(c){var io=crCaseIO(c);if(io.noOutput)return 0;var et=io.ej?io.ej.total:io.exp,at=io.aj?io.aj.total:io.act;var e=num(et),a=num(at);if(isNaN(e)||isNaN(a))return 0;if(cur(et)!==cur(at))return 0;return Math.abs(a-e)<=Math.abs(e)*0.01?100:0;}
  var CONTENT_SCORERS={'u-seed-invoice-fields':fieldsScore,'u-seed-total-tolerance':toleranceScore};
  var _cs=cellScore;cellScore=function(gi,mk,ok){var f=CONTENT_SCORERS[mk];if(f){var c=crCaseAt(gi);if(c)return f(c);}return _cs.apply(this,arguments);};
  var _fmd=freezeMetricData;freezeMetricData=function(){var out=_fmd.apply(this,arguments);out.forEach(function(x){var f=CONTENT_SCORERS[x.k];if(!f)return;var vals=[];allCases().forEach(function(c){try{if(!caseMetricEligible(x,c))return;}catch(e){}vals.push(f(c));});if(vals.length)x._score=Math.round(vals.reduce(function(a,b){return a+b},0)/vals.length);});return out;};
  var _after4=dsAfterModes;dsAfterModes=function(){try{if((APP_MODE==='hub'||APP_MODE==='console')&&localStorage.getItem('humain_seed_rev')!==String(SEED_REV)&&!/[?&]seed=0/.test(location.search)){var had=BASE_RUNS.length||BASELINES.length||DATASETS.length;Object.keys(localStorage).filter(function(k){return k.indexOf('eval_journey_')===0}).forEach(function(k){localStorage.removeItem(k)});localStorage.setItem('humain_seed_rev',String(SEED_REV));if(had){location.reload();return;}}}catch(e){}_after4.apply(this,arguments);};
})();

/* V103: flag reasons name the wrong field instead of the generic "Incomplete output" */
function crFieldDiffs(c){var io=crCaseIO(c);if(io.noOutput||!io.ej||!io.aj)return null;var out=[];Object.keys(io.ej).forEach(function(k){var e=io.ej[k],a=io.aj[k];if(crNorm(e)===crNorm(a))return;var label=k.replace(/_/g,' ');var en=String(e),an=String(a==null?'':a);
  if(k==='total'){var ne=parseFloat(en.replace(/,/g,'')),na=parseFloat(an.replace(/,/g,''));var ce=(en.match(/\b(SAR|USD|EUR|AED)\b/i)||[''])[0],ca=(an.match(/\b(SAR|USD|EUR|AED)\b/i)||[''])[0];
    if(!isNaN(ne)&&ne===na&&ce&&ca&&ce.toUpperCase()!==ca.toUpperCase()){out.push({short:'Wrong currency',text:'Wrong currency · '+ca+' instead of '+ce});return;}
    if(!isNaN(ne)&&ne===na){out.push({short:'Total incomplete',text:'Total incomplete · "'+an+'" instead of "'+en+'"'});return;}}
  if(a==null||an==='')out.push({short:'Missing '+label,text:'Missing '+label+' · expected '+en});else out.push({short:'Wrong '+label,text:'Wrong '+label+' · '+an+' instead of '+en});});return out;}
(function(){
  var _ar=attentionReasons;
  attentionReasons=function(c,i,metrics){var out=_ar.apply(this,arguments);var ix=out.findIndex(function(r){return r.kind==='execution'&&r.label==='Incomplete output'});if(ix<0)return out;var d=crFieldDiffs(c);if(!d)return out;
    if(!d.length){out.splice(ix,1);if(!out.some(function(r){return r.kind!=='control'}))out.push({kind:'control',label:'No review flag'});return out;}
    out.splice.apply(out,[ix,1].concat(d.map(function(x){return {kind:'execution',label:x.text,short:x.short}})));return out;};
  if(typeof outputLabel==='function'){var _ol=outputLabel;outputLabel=function(c){var r=_ol.apply(this,arguments);if(r&&r[0]==='incomplete'){var d=crFieldDiffs(c);if(d&&d.length)return [d.length===1?d[0].short.toLowerCase():d.length+' fields wrong','s-lo'];}return r;};}
})();

/* V104: one decision bar for the case review (decide, then move on, without hunting for buttons) */
function dbEnsure(){var s3=document.getElementById('s3');if(!s3)return null;var bar=document.getElementById('caseDecisionBar');if(s3&&!s3.classList.contains('has-dbar'))s3.classList.add('has-dbar');
  if(!bar){bar=document.createElement('div');bar.id='caseDecisionBar';bar.className='dbar';bar.innerHTML='<div class="dbar-state" id="dbState"></div><div class="dbar-acts" id="dbActs"></div><div class="dbar-nav" id="dbNav"></div>';s3.appendChild(bar);
    var acts=bar.querySelector('#dbActs');['saveNote','skipCase','cancelEdit'].forEach(function(id){var b=document.getElementById(id);if(b)acts.appendChild(b);});var extra=document.createElement('span');extra.id='dbExtra';extra.className='dbar-extra';acts.appendChild(extra);}
  return bar;}
function renderDecisionBar(){var bar=dbEnsure();if(!bar)return;var c;try{c=currentReviewCase();}catch(e){c=null;}if(!c){bar.hidden=true;return;}bar.hidden=false;
  var i=traceGlobal,total=totalCases(),stage=CASE_REVIEW_STAGE,saved=stage===3,pending=resultCaseSummary().pending.length,st;
  try{st=humanReviewState(c,i);}catch(e){st='';}
  var reasons=[];try{reasons=attentionReasons(c,i).filter(function(r){return r.kind!=='control'}).map(function(r){return r.short||r.label});}catch(e){}
  var dismissed=c.noteState==='dismissed'||c.skipped;
  document.getElementById('dbState').innerHTML='<b>Case '+(i+1)+' of '+total+'</b><span class="dbar-st '+(saved?(dismissed?'ok':'bad'):'')+'">'+esc(saved?(dismissed?'Marked acceptable':'Problem confirmed'):(st||'Not reviewed'))+'</span>'+(reasons.length&&!saved?'<small>'+esc(reasons.slice(0,2).join(' · '))+'</small>':'');
  var acts=document.getElementById('dbActs');acts.classList.toggle('saved',saved);var sk=document.getElementById('skipCase');if(sk&&sk.textContent.trim()!=='Mark acceptable')sk.textContent='Mark acceptable';
  document.getElementById('dbExtra').innerHTML=saved?'<button type="button" class="btn ghost sm" data-review-stage="2">Edit decision</button>':'';
  dbPlace();
  document.getElementById('dbNav').innerHTML='<button type="button" class="btn ghost sm" id="dbPrev" title="Previous case" aria-label="Previous case">&lsaquo;</button><button type="button" class="btn ghost sm" id="dbNext" title="Next case" aria-label="Next case">&rsaquo;</button><button type="button" class="btn sm'+(saved?'':' ghost')+'" id="dbNextUnread"'+(pending?'':' disabled')+' title="Open the next case that still needs a decision">Next to review'+(pending?' · '+pending:'')+'</button><button type="button" class="btn ghost sm dbar-update" data-update-case="'+i+'" title="Fix the agent and upload it as a new version">Update agent</button>';
}
var DB_TRIES=0;
function dbPlace(){var bar=document.getElementById('caseDecisionBar'),s3=document.getElementById('s3');if(!bar||!s3||bar.hidden)return;var r=s3.getBoundingClientRect();if(!r.width){if(DB_TRIES++<20)setTimeout(dbPlace,40);return;}DB_TRIES=0;bar.style.setProperty('--dbar-left',Math.round(r.left)+'px');bar.style.setProperty('--dbar-width',Math.round(r.width)+'px');requestAnimationFrame(function(){s3.style.paddingBottom=(bar.offsetHeight+24)+'px';});}
window.addEventListener('resize',dbPlace);
(function(){
  var _rcr=renderCaseReview;renderCaseReview=function(){var r=_rcr.apply(this,arguments);try{renderDecisionBar();}catch(e){}return r;};
  var _rc=renderCase;renderCase=function(){var r=_rc.apply(this,arguments);try{renderDecisionBar();setTimeout(dbPlace,60);}catch(e){}return r;};
  window.addEventListener('click',function(e){var t=e.target.closest&&e.target.closest('#dbPrev,#dbNext,#dbNextUnread');if(!t)return;e.preventDefault();e.stopImmediatePropagation();
    if(t.id==='dbPrev'){var p=document.getElementById('prevCase');if(p)p.click();}else if(t.id==='dbNext'){var n=document.getElementById('nextCase');if(n)n.click();}else{nextUnreviewed();}},true);
})();

/* V105: embedded chrome. Inside the portal the engine drops its own header (Demo tools stay as a small chip); scoped views drop the nav column. */
(function(){try{if(window.parent!==window)document.body.classList.add('embedded');}catch(e){}})();

/* V106: in the scoped embedded view the nav is gone, so the results page carries Update agent itself */
(function(){
  function placeUpdate(){if(!HR_SCOPE||APP_MODE!=='console')return;var back=[...document.querySelectorAll('#s2 [data-nav-dest="run"]')].filter(function(b){return b.offsetParent!==null||true})[0];if(!back)return;if(document.getElementById('hrUpdateAgentMain'))return;var b=document.createElement('button');b.type='button';b.className='btn';b.id='hrUpdateAgentMain';b.textContent='Update agent';b.title='Fix the agent, upload it as a new version, evaluate and submit again';back.insertAdjacentElement('afterend',b);}
  var _sm4=syncModeChrome;syncModeChrome=function(){var r=_sm4.apply(this,arguments);try{placeUpdate();}catch(e){}return r;};
  window.addEventListener('click',function(e){var t=e.target.closest&&e.target.closest('#hrUpdateAgentMain');if(!t)return;e.preventDefault();e.stopImmediatePropagation();goToSubmitAgent();},true);
})();

/* V107: Update agent where the flagged cases are reviewed (results decision banner and Cases card), not only in the nav */
(function(){
  function ensure(){var s2=document.getElementById('s2');if(!s2)return;
    var banner=document.getElementById('resDecision');if(banner&&!banner.querySelector('.hr-update-here')){var hero=document.getElementById('reviewNextHero');if(hero){var b=document.createElement('button');b.type='button';b.className='btn ghost hr-update-here';b.textContent='Update agent';b.title='Fix the agent and upload it as a new version';hero.insertAdjacentElement('afterend',b);}}
    var hd=document.querySelector('#resultsCases .hd');if(hd&&!hd.querySelector('.hr-update-here')){var rn=document.getElementById('reviewNext');if(rn){var b2=document.createElement('button');b2.type='button';b2.className='btn ghost sm hr-update-here';b2.textContent='Update agent';rn.insertAdjacentElement('afterend',b2);}}}
  var s2=document.getElementById('s2');if(s2){new MutationObserver(function(){ensure();}).observe(s2,{childList:true,subtree:true});}
  ensure();
  window.addEventListener('click',function(e){var t=e.target.closest&&e.target.closest('.hr-update-here');if(!t)return;e.preventDefault();e.stopImmediatePropagation();goToSubmitAgent();},true);
})();

/* V109: inside the portal the hub navigation lives in the portal sidebar; the engine reports its current page */
(function(){
  if(window.parent===window)return;
  var _sm5=syncModeChrome;syncModeChrome=function(){var r=_sm5.apply(this,arguments);try{shellPost('humain-eval-page',{page:APP_MODE==='hub'?HUB_PAGE:'console'});}catch(e){}return r;};
})();
