"""V95: datasets as named, versioned tables in the hub. Built from V64 by exact-match edits."""
...
def rep(old,new,n=1,tag=''):
    ...
    s=s.replace(old,new)
rep('HUMAIN ONE Evaluate - V64','HUMAIN ONE Evaluate - V95',1,'wm')
rep("var KEY='eval_journey_v64';","var KEY='eval_journey_v95';",1,'key')
rep("skey().replace('eval_journey_v64','eval_journey_v63')","skey().replace('eval_journey_v95','eval_journey_v94')",2,'fallback key')
rep("version:'V64'","version:'V95'",1,'shell version')
...
JS=open(SP+'datasets_v95.js',encoding='utf-8').read()
rep("installModes();\n\n</script>\n</body>", JS+"\ninstallModes();\ndsAfterModes();\n</script>\n</body>",1,'datasets script')
CSS=open(SP+'datasets_v95.css',encoding='utf-8').read()
...
open(ROOT+'Eval_Journey_V95.html','w',encoding='utf-8').write(s)
print('built V95',len(s))
