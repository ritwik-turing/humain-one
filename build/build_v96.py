"""V96: datasets as named, versioned tables in the hub. Built from V64 by exact-match edits."""
...
def rep(old,new,n=1,tag=''):
    ...
    s=s.replace(old,new)
rep('HUMAIN ONE Evaluate - V64','HUMAIN ONE Evaluate - V96',1,'wm')
rep("var KEY='eval_journey_v64';","var KEY='eval_journey_v96';",1,'key')
rep("skey().replace('eval_journey_v64','eval_journey_v63')","skey().replace('eval_journey_v96','eval_journey_v95')",2,'fallback key')
rep("version:'V64'","version:'V96'",1,'shell version')
...
JS=open(SP+'datasets_v96.js',encoding='utf-8').read()
rep("installModes();\n\n</script>\n</body>", JS+"\ninstallModes();\ndsAfterModes();\n</script>\n</body>",1,'datasets script')
CSS=open(SP+'datasets_v96.css',encoding='utf-8').read()
...
open(ROOT+'Eval_Journey_V96.html','w',encoding='utf-8').write(s)
print('built V96',len(s))
