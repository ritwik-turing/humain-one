"""V98: datasets as named, versioned tables in the hub. Built from V64 by exact-match edits."""
...
def rep(old,new,n=1,tag=''):
    ...
    s=s.replace(old,new)
rep('HUMAIN ONE Evaluate - V64','HUMAIN ONE Evaluate - V98',1,'wm')
rep("var KEY='eval_journey_v64';","var KEY='eval_journey_v98';",1,'key')
rep("skey().replace('eval_journey_v64','eval_journey_v63')","skey().replace('eval_journey_v98','eval_journey_v97')",2,'fallback key')
rep("version:'V64'","version:'V98'",1,'shell version')
...
JS=open(SP+'datasets_v98.js',encoding='utf-8').read()
rep("installModes();\n\n</script>\n</body>", JS+"\ninstallModes();\ndsAfterModes();\n</script>\n</body>",1,'datasets script')
CSS=open(SP+'datasets_v98.css',encoding='utf-8').read()
...
open(ROOT+'Eval_Journey_V98.html','w',encoding='utf-8').write(s)
print('built V98',len(s))
