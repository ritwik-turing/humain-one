"""V99: datasets as named, versioned tables in the hub. Built from V64 by exact-match edits."""
...
def rep(old,new,n=1,tag=''):
    ...
    s=s.replace(old,new)
rep('HUMAIN ONE Evaluate - V64','HUMAIN ONE Evaluate - V99',1,'wm')
rep("var KEY='eval_journey_v64';","var KEY='eval_journey_v99';",1,'key')
rep("skey().replace('eval_journey_v64','eval_journey_v63')","skey().replace('eval_journey_v99','eval_journey_v98')",2,'fallback key')
rep("version:'V64'","version:'V99'",1,'shell version')
...
JS=open(SP+'datasets_v99.js',encoding='utf-8').read()
rep("installModes();\n\n</script>\n</body>", JS+"\ninstallModes();\ndsAfterModes();\n</script>\n</body>",1,'datasets script')
CSS=open(SP+'datasets_v99.css',encoding='utf-8').read()
...
open(ROOT+'Eval_Journey_V99.html','w',encoding='utf-8').write(s)
print('built V99',len(s))
