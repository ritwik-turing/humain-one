...
function dbEnsure(){var s3=document.getElementById('s3');if(!s3)return null;var bar=document.getElementById('caseDecisionBar');
...
function renderDecisionBar(){var bar=dbEnsure();if(!bar)return;var c;try{c=currentReviewCase();}catch(e){c=null;}if(!c){bar.hidden=true;return;}bar.hidden=false;
  ...
  var acts=document.getElementById('dbActs');acts.classList.toggle('saved',saved);
  ...
  dbPlace();
  document.getElementById('dbNav').innerHTML='<button type="button" class="btn ghost sm" id="dbPrev" aria-label="Previous case">&lsaquo; Previous</button><button type="button" class="btn ghost sm" id="dbNext" aria-label="Next case">Next &rsaquo;</button><button type="button" class="btn sm'+(saved?'':' ghost')+'" id="dbNextUnread"'+(pending?'':' disabled')+'>Review next case'+(pending?' · '+pending+' left':'')+'</button><button type="button" class="btn ghost sm dbar-update" data-update-case="'+i+'" title="Fix the agent and upload it as a new version">Update agent</button>';
}
var DB_TRIES=0;
