...
const OUT = path.join(REPO, 'Agent_Lifecycle_V55.html');
const FOCUS_CSS = "\n/* V55 focus layout: slim step rail, value rail collapsed by default */\n.rail{position:relative}\n.rail-toggle{position:absolute;right:-13px;top:22px;width:26px;height:26px;border-radius:50%;border:1px solid var(--line);background:#fff;color:var(--ink2);font:inherit;font-size:15px;line-height:1;cursor:pointer;z-index:3;box-shadow:0 1px 3px rgba(0,0,0,.08);display:grid;place-items:center;padding:0}\n.rail-toggle:hover{color:var(--green-d);border-color:var(--green)}\n.app.railslim{grid-template-columns:64px minmax(0,1fr) 344px}\n.app.railslim.railhide,.app.railslim.developer-eval{grid-template-columns:64px minmax(0,1fr)}\n.app.railslim .brand{justify-content:center;padding:16px 0 12px}.app.railslim .brand>div:not(.logo){display:none}\n.app.railslim .mode,.app.railslim .nav-title,.app.railslim .rail-foot,.app.railslim .stage .nm{display:none}\n.app.railslim .stages{padding:10px 8px}.app.railslim .stage{justify-content:center;padding:9px 0;gap:0}.app.railslim .stage.active::before{display:none}\n.app.developer-eval .canvas{max-width:1400px}\n@media(max-width:980px){.app.railslim{grid-template-columns:64px minmax(0,1fr)}}\n@media(max-width:700px){.app.railslim.developer-eval{grid-template-columns:1fr}}\n";
const FOCUS_JS = "\n/* V55: focus layout defaults and the rail toggle */\n(function(){\n  var app=document.querySelector('.app');if(!app)return;var LS=window.localStorage;\n  function pref(k,d){try{var v=LS.getItem(k);return v==null?d:v;}catch(e){return d;}}\n  if(pref('humain_valuerail','0')!=='1')app.classList.add('railhide');\n  if(pref('humain_railslim','1')==='1')app.classList.add('railslim');\n  var rail=document.querySelector('.rail');\n  if(rail&&!document.getElementById('railSlimToggle')){var b=window.document.createElement('button');b.id='railSlimToggle';b.className='rail-toggle';b.type='button';rail.insertBefore(b,rail.firstChild);}\n  function paint(){var slim=app.classList.contains('railslim');var t=document.getElementById('railSlimToggle');if(t){t.innerHTML=slim?'&rsaquo;':'&lsaquo;';t.title=slim?'Show the menu':'Collapse the menu';t.setAttribute('aria-label',t.title);}}\n  paint();\n  __ROOT__.addEventListener('click',function(e){var t=e.target.closest&&e.target.closest('#railSlimToggle');if(t){app.classList.toggle('railslim');try{LS.setItem('humain_railslim',app.classList.contains('railslim')?'1':'0');}catch(_){}paint();return;}\n    var v=e.target.closest&&e.target.closest('.vreopen,[onclick*=\"toggleRail\"]');if(v){setTimeout(function(){try{LS.setItem('humain_valuerail',app.classList.contains('railhide')?'0':'1');}catch(_){}},0);}});\n})();\n";
const ENGINE = 'Eval_Journey_V104.html';
...
LJ = rep(LJ, '<li class="stage ${cls}" data-i="${i}">', '<li class="stage ${cls}" data-i="${i}" title="${s.n}">', 1, 'stage tooltips');
if(!/\}\)\(\);\s*$/.test(LJ)) fails.push('focus init tail'); else LJ = LJ.replace(/\}\)\(\);\s*$/, FOCUS_JS + ' })();');
// ---- V55: Submit Agent flow is the Invoice Processing Agent end to end ----
...
// ---------------- shell ----------------
H = rep(H, '<title>HUMAIN ONE — Agent Lifecycle V13</title>', '<title>HUMAIN ONE — Agent Lifecycle V55</title>', 1, 'title');
...
window.__evalHub=(function(){
  var ENGINE='${ENGINE}',KEYS=['eval_journey_v104:final','eval_journey_v103:final'];
...
H = rep(H, 'var __LC="', 'var __LC="' + JSON.stringify(FOCUS_CSS).slice(1,-1), 1, 'focus css');
H = rep(H, '/* ---------- ROUTER ---------- */', "/* V55: collapse the portal nav on the working screens unless the user expanded it */\ndocument.addEventListener('DOMContentLoaded',function(){function focusNav(s){try{var app=document.getElementById('app');if(!app)return;var pref=null;try{pref=localStorage.getItem('h1-collapsed');}catch(e){}if((s==='submit'||s==='evaluations')&&pref!=='0'){app.classList.add('collapsed');document.body.classList.add('nav-collapsed');}}catch(e){}}try{var _g3=window.go;window.go=go=function(s){_g3(s);focusNav(s);};}catch(e){}setTimeout(function(){try{focusNav(state.screen);}catch(e){}},80);});\n/* ---------- ROUTER ---------- */", 1, 'portal nav focus');
