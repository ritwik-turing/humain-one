...
const OUT = path.join(REPO, 'Agent_Lifecycle_V48.html');
const ENGINE = 'Eval_Journey_V98.html';
...
// ---- V48: Submit Agent flow is the Invoice Processing Agent end to end ----
...
LJ = rep(LJ, "return {slug:a.slug||'al-noor-services',ver:a.ver||'v1.4.0'}; }", "return {slug:a.slug||'invoice-processing',ver:a.ver||'v0.9.0',name:a.name||'Invoice Processing Agent',av:a.av||'IP',owner:a.owner||'Ministry of Finance',id:a.id||'inv'}; }", 1, 'agentMeta defaults');
...
// ---------------- shell ----------------
H = rep(H, '<title>HUMAIN ONE — Agent Lifecycle V13</title>', '<title>HUMAIN ONE — Agent Lifecycle V48</title>', 1, 'title');
...
window.__reviews=(function(){
  ...
  function panel(){resolve();var list=load().slice().sort(function(a,b){return b.submittedAt-a.submittedAt});
    var rows=list.map(function(r){var p=PILL[r.status]||PILL.queued,fb='';
      ...
      return '<tr class="rv-'+r.status+'-row"><td><b>'+esc(r.name)+'</b><span class="rv-sub">'+esc(r.ver)+'</span></td><td>Evaluation '+r.evaluation+'<span class="rv-sub">'+esc(r.experimentset)+' · '+r.cases+' cases · average '+r.avg+'</span></td><td class="rv-when">'+when(r.submittedAt)+'</td><td><span class="rv-pill '+p[1]+'">'+p[0]+'</span><span class="rv-sub">'+REVIEWER+'</span></td><td class="rv-fbcell">'+fb+'</td></tr>';}).join('');
...
window.__evalHub=(function(){
  var ENGINE='${ENGINE}',KEYS=['eval_journey_v98:final','eval_journey_v97:final'];
