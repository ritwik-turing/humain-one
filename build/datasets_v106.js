...
function renderDecisionBar(){var bar=dbEnsure();if(!bar)return;var c;try{c=currentReviewCase();}catch(e){c=null;}if(!c){bar.hidden=true;return;}bar.hidden=false;
  ...
  document.getElementById('dbExtra').innerHTML=saved?'<button type="button" class="btn ghost sm" data-review-stage="2">Edit decision</button>'+(dismissed?'':'<button type="button" class="btn ghost sm" data-update-case="'+i+'">Update agent</button>'):'';
  dbPlace();
