...
- Prototype, current: https://ritwik-turing.github.io/humain-one/Eval_Journey_V11.html (file Eval_Journey_V11.html). V11 preserves V10's corrected Prism metric model and adds the Humain ONE reproducibility layer: an immutable run receipt pins the agent, dataset snapshot, metric definition IDs and versions, evaluator execution, and run settings. Results, like-for-like comparison, evidence, and verification all carry that receipt.
...
- State-of-the-art benchmark and V11 release bar: EVAL_STATE_OF_THE_ART_AUDIT.md.
