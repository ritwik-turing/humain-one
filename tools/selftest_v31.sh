#!/bin/bash
# V31 acceptance gate: outcome-first end-to-end evaluation journey.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TARGET="${1:-$SCRIPT_DIR/../Eval_Journey_V31.html}"
echo "V31 acceptance gate: current progressive-disclosure contracts"

python3 - "$TARGET" <<'PY'
import re, sys
page=open(sys.argv[1],encoding='utf-8').read()
checks={
  'V31 is a separate versioned artifact':'Evaluate V31' in page,
...
print(f'V31 static contract: {len(checks)-len(failed)} of {len(checks)} passed')
...
const dom=new JSDOM(page,{runScripts:'dangerously',url:'https://v31.test',pretendToBeVisual:true,virtualConsole:vc});
setTimeout(async()=>{
  ...
  console.log(`V31 DOM contract: ${out.length-failed.length} of ${out.length} passed`);
