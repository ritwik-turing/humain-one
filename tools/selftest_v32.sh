...
setTimeout(async()=>{
  ...
  test('no-data Quality opens with one prerequisite and zero draft metrics',()=>{w.navigateSetup('agentSetup','replace');const on=w.allMetrics().concat(w.EXTRA_METRICS).filter(m=>m.on);return on.length===0&&!w.document.getElementById('qualityPrereqState').hidden&&w.document.querySelector('#agentSetup .quality-nav').hidden&&w.document.getElementById('navMetricStatus').textContent==='Not started'&&w.document.querySelector('#agentSetup .panel-next').hidden;});
  ...
  test('regression metric starts with reviewed-evidence guidance only',()=>!w.document.getElementById('regressionStartState').hidden&&w.document.getElementById('regressionWorkspace').hidden&&w.document.querySelector('#s5 .section-map').hidden);
