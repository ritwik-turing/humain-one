#!/bin/bash
# V32 acceptance gate: outcome-first end-to-end evaluation journey.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TARGET="${1:-$SCRIPT_DIR/../Eval_Journey_V32.html}"
echo "V32 acceptance gate: current progressive-disclosure contracts"

python3 - "$TARGET" <<'PY'
import re, sys
page=open(sys.argv[1],encoding='utf-8').read()
checks={
  'V32 is a separate versioned artifact':'Evaluate V32' in page,
  'setup opens as one three-step decision flow':all(x in page for x in ('<div class="sgrp">Setup evaluation</div>','id="setupTitle">Add your evaluation data','class="setup-progress"','id="dataSetup" open','id="agentSetup"','id="runSetup"')),
  'first use starts unattached and exposes an explicit add dataset action':all(x in page for x in ('var DATA_ATTACHED=false','id="dataEmptyState"','id="addDataset"','No dataset attached','Nothing is attached to this evaluation until you approve the preview')),
  'first use starts with zero deliberately attached domain metrics':all(x in page for x in ("var EVAL_NAME='Al Noor mixed eval v3', EVAL_DESC='', NEW_EVAL=true","{k:'acc',  on:false","{k:'trust',on:false","{k:'help', on:false","{k:'traj', on:false")),
  'Quality uses friendly prerequisite and zero-check starting states':all(x in page for x in ('id="qualityPrereqState"','id="qualityStartState"','Add evaluation data before choosing checks','Define what a good answer means for this agent')),
  'Review and run hides the receipt behind readiness':all(x in page for x in ('id="runPrereqState"','id="runWorkspace" hidden','Run with platform evidence only','Not ready to run')),
  'completed job metrics are seeded independently from the draft':'function freezeSeededCompletedJobMetricData' in page and "SNAP.id==='cfg-9D4E7B2'?freezeSeededCompletedJobMetricData():freezeMetricData()" in page,
  'metric creation is a real staged flow':all(x in page for x in ('data-builder-next="2"','data-builder-next="3"','data-builder-next="4"',"stage-'+stage")),
  'regression attachment is blocked until recalibration':all(x in page for x in ('id="deferSave" disabled','if(!CHECK_CALIBRATED)','Retest revised check')),
  ...
  'top-level setup navigation returns to the page origin':"if(isTopLevel)window.scrollTo({top:0,behavior:'smooth'})" in page,
...
print(f'V32 static contract: {len(checks)-len(failed)} of {len(checks)} passed')
...
const dom=new JSDOM(page,{runScripts:'dangerously',url:'https://v32.test',pretendToBeVisual:true,virtualConsole:vc});
setTimeout(async()=>{
  ...
  test('setup opens with one three-step decision flow',()=>w.document.querySelector('#s1 .h1').textContent.trim()==='Add your evaluation data'&&w.document.getElementById('dataSetup').open&&!w.document.getElementById('agentSetup').open&&w.document.querySelectorAll('.setup-progress button').length===3&&w.document.querySelectorAll('.setup-progress [aria-current="step"]').length===1);
  test('first use has no attached dataset or premature replacement controls',()=>w.DATA_ATTACHED===false&&!w.document.getElementById('dataEmptyState').hidden&&w.document.querySelector('#dataSetup .data-receipt').hidden&&w.document.querySelector('#dataSetup .data-actions').hidden&&w.document.getElementById('navDataMeta').textContent==='No dataset'&&w.document.getElementById('navDataStatus').textContent==='Not started'&&w.document.querySelector('#dataSetup .panel-next').hidden);
  test('Add dataset reveals source choices without attaching anything',()=>{w.document.getElementById('addDataset').click();return w.document.getElementById('importSources').open&&!w.DATA_ATTACHED&&/Upload CSV/.test(w.document.getElementById('importSources').textContent)&&/Connect to Data Warehouse/.test(w.document.getElementById('importSources').textContent);});
  ...
  test('no-data Quality opens only the prerequisite and zero draft metrics',()=>{w.navigateSetup('agentSetup','replace');const on=w.allMetrics().concat(w.EXTRA_METRICS).filter(m=>m.on);return on.length===0&&!w.document.getElementById('qualityPrereqState').hidden&&w.document.getElementById('qualityStartState').hidden&&w.document.querySelector('#agentSetup .quality-nav').hidden&&w.document.getElementById('navMetricStatus').textContent==='Not started'&&w.document.querySelector('#agentSetup .panel-next').hidden;});
  test('no-data Review and run names the unmet prerequisites honestly',()=>{w.navigateSetup('runSetup','replace');const list=w.document.getElementById('runReadiness');return !w.document.getElementById('runPrereqState').hidden&&w.document.getElementById('runWorkspace').hidden&&/Waiting for a dataset/.test(list.textContent)&&/Input mapping comes after data/.test(list.textContent)&&list.querySelectorAll('.readiness-item.done').length===0;});
  test('completed job retains four frozen quality metrics independently',()=>{const m=w.jobMetrics();return m.length===4&&m.every(x=>x._coverage&&x._coverage.total===380&&x._coverage.ran>0)&&w.snapshotMetricKeys().length===4;});
  ...
  test('no-data Quality cannot expose later configuration views',()=>{w.openQualityView('platform',true,'push');return w.ACTIVE_QUALITY_VIEW==='attached'&&!w.document.getElementById('qualityPrereqState').hidden&&w.document.getElementById('platformEvidenceView').hidden;});
  test('warehouse connection has named current and future steps',()=>{w.navigateSetup('dataSetup','replace');w.document.getElementById('addDataset').click();w.document.getElementById('connectWarehouse').click();const labels=[...w.document.querySelectorAll('.wizard-steps li')].map(x=>x.textContent.trim());return labels.join('|')==='Source|Access|Select data|Map fields'&&w.document.querySelectorAll('.wizard-steps [aria-current="step"]').length===1&&/Step 1 of 4/.test(w.document.querySelector('.wizard-progress-head').textContent);});
  w.document.querySelector('[data-wh-provider="snowflake"]').click();w.document.getElementById('whTest').click();await new Promise(resolve=>setTimeout(resolve,720));w.document.querySelector('[data-wh-asset="unknown"]').click();w.document.getElementById('whUse').click();await new Promise(resolve=>setTimeout(resolve,820));
  test('approved Snowflake preview creates the first attached draft dataset',()=>w.DATA_ATTACHED===true&&w.document.getElementById('dataEmptyState').hidden&&!w.document.querySelector('#dataSetup .data-receipt').hidden&&w.document.querySelector('#dataSetup .panel-next').hidden&&w.document.getElementById('navDataMeta').textContent==='380 cases'&&w.document.getElementById('navDataStatus').textContent==='Needs mapping'&&w.document.getElementById('dataContractStatus').textContent==='Review required');
  test('Replace source appears only after the first dataset exists',()=>{w.document.getElementById('replaceSource').click();return w.document.getElementById('importSources').open&&/Safe replacement/.test(w.document.getElementById('sourceGuard').textContent);});
  w.document.getElementById('confirmMapping').click();w.navigateSetup('agentSetup','replace');
  test('mapped Quality starts with one friendly zero-check choice',()=>w.allMetrics().concat(w.EXTRA_METRICS).filter(m=>m.on).length===0&&!w.document.getElementById('qualityStartState').hidden&&w.document.querySelector('#agentSetup .quality-nav').hidden&&/Define what a good answer means/.test(w.document.getElementById('qualityStartState').textContent));
  test('Add quality check opens the library in context',()=>{w.document.querySelector('#qualityStartState [data-quality-view="library"]').click();return w.document.getElementById('qualityStartState').hidden&&!w.document.querySelector('#agentSetup .quality-nav').hidden&&!w.document.getElementById('qualityCheckPicker').hidden&&!w.document.getElementById('qualityLibraryPane').hidden&&w.document.getElementById('qualityCreatePane').hidden;});
  test('quality subnavigation exposes platform evidence in one click',()=>{w.openQualityView('platform',true,'push');const active=w.document.querySelector('.quality-nav [aria-current="location"]'),p=w.document.getElementById('platformEvidenceView');return active&&active.dataset.qualityView==='platform'&&!p.hidden&&w.document.getElementById('qualityAttachedView').hidden&&w.document.getElementById('qualityCheckPicker').hidden&&/view=platform/.test(w.location.search);});
  test('quality library is exclusive and restores a shareable view',()=>{w.openQualityView('library',true,'push');return w.document.getElementById('qualityStartState').hidden&&!w.document.querySelector('#agentSetup .quality-nav').hidden&&!w.document.getElementById('qualityCheckPicker').hidden&&!w.document.getElementById('qualityLibraryPane').hidden&&w.document.getElementById('qualityCreatePane').hidden&&w.document.getElementById('qualityAttachedView').hidden&&w.document.getElementById('platformEvidenceView').hidden&&/view=library/.test(w.location.search);});
  ...
  test('regression metric starts with one primary and no duplicate footer route',()=>!w.document.getElementById('regressionStartState').hidden&&w.document.getElementById('regressionWorkspace').hidden&&w.document.querySelector('#s5 .section-map').hidden&&!w.document.querySelector('#s5 .workflow-footer')&&w.document.querySelectorAll('#regressionStartState .btn:not(.ghost)').length===1);
  test('regression attachment stays blocked until revision and retest',()=>{w.document.getElementById('startRegression').click();const save=w.document.getElementById('deferSave'),blocked=save.disabled;const draft=w.document.getElementById('chkDraft');draft.value+=' Require every requested item to be processed.';draft.dispatchEvent(new w.Event('input',{bubbles:true}));w.document.getElementById('deferTry').click();return blocked&&w.CHECK_CALIBRATED===true&&!save.disabled&&/Save v1.0.0 and attach/.test(save.textContent);});
  ...
  test('tablet and mobile workflow drawer focuses, traps, and closes on Escape',()=>{Object.defineProperty(w,'innerWidth',{value:768,configurable:true});const toggle=w.document.getElementById('workflowMenuToggle');toggle.focus();toggle.click();const close=w.document.getElementById('workflowMenuClose'),opened=w.document.body.classList.contains('nav-open')&&w.document.activeElement===close;close.dispatchEvent(new w.KeyboardEvent('keydown',{key:'Escape',bubbles:true}));const ok=opened&&!w.document.body.classList.contains('nav-open')&&w.document.activeElement===toggle;if(!ok)throw new Error('opened='+opened+' navOpen='+w.document.body.classList.contains('nav-open')+' focus='+(w.document.activeElement&&w.document.activeElement.id));return true;});
  ...
  test('metric builder starts with only the criterion decision',()=>{w.MB={type:'judge',stage:1};w.document.getElementById('mbuild').hidden=false;w.renderBuilder();const b=w.document.querySelector('#mbuild .mb');return b.classList.contains('stage-1')&&!!b.querySelector('[data-builder-next="2"]');});
  test('metric builder unlocks evidence and then method one step at a time',()=>{w.document.querySelector('[data-builder-next="2"]').click();const evidence=w.document.querySelector('#mbuild .mb').classList.contains('stage-2')&&!!w.document.getElementById('mbReference');w.document.querySelector('[data-builder-next="3"]').click();return evidence&&w.document.querySelector('#mbuild .mb').classList.contains('stage-3')&&!!w.document.querySelector('[data-mtype="judge"]')&&w.MB.stage===3;});
  ...
  test('no-reference choice disclaims factual truth',()=>{w.MB.reference='none';w.MB.stage=2;w.renderBuilder();return /quality, not factual truth/i.test(w.document.querySelector('.reference-choice').textContent);});
  ...
  test('a new evaluation starts with zero attached domain metrics',()=>{const on=w.allMetrics().concat(w.EXTRA_METRICS).filter(m=>m.on).map(m=>m.n);const txt=w.document.getElementById('metricContext').textContent;if(on.length||!/No quality checks attached/.test(txt))throw new Error('on='+on.join(',')+' text='+txt);return true;});
  test('detaching draft checks preserves the reusable library and completed receipt',()=>w.allMetrics().length+w.EXTRA_METRICS.length>=7&&w.allMetrics().filter(m=>m.on).length===0&&w.jobMetrics().length===4&&w.snapshotMetricKeys().length===4);
  ...
  w.show('s8');
  test('dirty draft surfaces its completed-job submit decision before package details',()=>{const b=w.document.getElementById('doSubmit'),lead=w.document.getElementById('submitDecision'),pkg=w.document.getElementById('submitPackage'),txt=w.document.getElementById('s8').textContent;return !b.disabled&&new RegExp('Submit job').test(b.textContent)&&/Draft changes excluded/.test(txt)&&/declared open issue/.test(w.document.getElementById('submitLeadTitle').textContent)&&!!(lead.compareDocumentPosition(pkg)&w.Node.DOCUMENT_POSITION_FOLLOWING);});
  ...
  console.log(`V32 DOM contract: ${out.length-failed.length} of ${out.length} passed`);
